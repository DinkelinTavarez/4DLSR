...
class MultiViewGeometry:
    def __init__(self, reference, occlusion_aware=False):
    ...
    def reconstruct(self, images):
        ...
        for i in range(len(images)):
            ...
            depths[i]*=factor;factors.append(float(factor))
        ...
        for i,(R,t) in enumerate(views):
            ...
            valid=np.isfinite(xyz).all(axis=1)&(depths[i].ravel()>0)&(conf[i].ravel()>np.percentile(conf[i],10))
            ...
            for other,(otherR,otherT) in enumerate(views):
                ...
                reliable=conf[other][v,u]>np.percentile(conf[other],10)
                keep,consistent,occluded=cross_view_visibility(cam[:,2],expected,inside,reliable)
                if not self.occlusion_aware:
                    consistent=inside&(np.abs(cam[:,2]-expected)/np.maximum(expected,.001)<.1)
                    keep=~inside|consistent;occluded=np.zeros_like(inside)
                supported+=consistent;contradicted+=~keep;hidden|=occluded
            # A majority of reliable contradictory views rejects the point;
            # unseen/occluded pixels are not votes for fabricated geometry.
            valid &= contradicted<=supported
        ...
        return points,colors,radii,dict(source='joint learned geometry',support=support,depthScaleCorrections=factors,initialGaussians=len(points))
