...
async function connect(targets){
  ...
  if(!selected.length)return;connecting=true;updateControls();const errors=[];
  // USB camera drivers are much less reliable when several identical devices
  // are opened at the same instant. Negotiate each physical stream fully before
  // asking Windows for the next one. This preserves the requested native mode;
  // it does not silently lower resolution or FPS.
  for(const [index,c] of selected.entries()){
    const error=await connectOne(c);if(error)errors.push(error);
    if(index<selected.length-1)await wait(350);
  }
  try{
...
function handleFailure(c,reason){if(!c.stream)return;const auto=$('auto-connect').checked;toast(`${c.label}: ${reason}. ${take?'Saving available footage.':auto?'Reconnecting automatically.':'Choose Connect selected cameras to retry.'}`);if(take){stop().then(()=>release(c)).catch(report);return;}release(c,false);c.retryCount++;c.retryAt=performance.now()+Math.min(30000,1000*2**Math.min(c.retryCount,5));$(`empty-${c.slot}`).textContent=auto?'Feed lost · reconnecting':'Feed lost · reconnect manually';$(`settings-${c.slot}`).textContent=reason;$('device-status').textContent=`${slots.filter(s=>s.stream).length} streams remain live. ${c.label}: ${reason}.`;updateControls();}
function disconnect(){if(take)return;$('auto-connect').checked=false;for(const c of slots)release(c);$('device-status').textContent=`Cameras disconnected. ${devices.length} detected; automatic connection is off until enabled or the page reloads.`;updateControls();}
...
async function autoRefresh(){if(simulated||!inventoryReady||take||stopping||connecting||scanning)return;await scan(false,true);if(!$('auto-connect').checked)return;const due=slots.filter(c=>!c.stream&&!c.connecting&&performance.now()>c.retryAt&&devices.some(d=>d.deviceId===$(`camera-${c.slot}`).value)).sort((a,b)=>a.retryAt-b.retryAt);if(due.length)await connect([due[0]]);}
...
$('auto-connect').checked=true;$('auto-connect').onchange=()=>{if($('auto-connect').checked)autoRefresh().catch(report);};
